        ...
        <tbody>
          ...
          @foreach ($order_details as $key => $details)
            ...
            <tr class="color_qty_{{ $details->product_id }}">
              ...
              <td>{{ number_format($details->product_price*$details->product_sales_quantity, 0,',','.') }}</td>
            </tr>
          @endforeach
            <tr>
              <td colspan="6">
                Tiền ban đầu: {{ number_format($total, 0,',','.').'đ' }}; 
                <br>         
                @php
                $total_coupon = 0;
                @endphp
                @if($coupon_condition==2 && $coupon_number!=0)
                  @php
                    $total_after_coupon = $total*$coupon_number/100;
                    echo 'Mã giảm giá 10%: '.number_format($total_after_coupon, 0,',','.').'đ'.'<br>';
                    $total_coupon = $total - $total_after_coupon + $details->product_feeship;
                  @endphp
                @elseif($coupon_condition==1 && $coupon_number!=0)
                  @php
                    echo 'Mã giảm giá theo tiền: '.number_format($coupon_number, 0,',','.').'đ'.'<br>';
                    $total_coupon = $total - $coupon_number + $details->product_feeship;
                  @endphp
                @elseif($coupon_condition==1 && $coupon_number==0)
                  @php
                    echo 'Không áp dụng mã giảm giá'.'<br>';
              ...
              <td colspan="6">
                ...
                <form>
                  ...
                  <select class="form-control order_details" >
                    ...
                    <option id="{{ $or->order_id }}" value="1">Chưa xử lý</option>
                    <option id="{{ $or->order_id }}" value="2" >Đã xử lý - Đã giao hàng</option>
                    <option id="{{ $or->order_id }}" value="3" selected >Hủy đơn hàng - Tạm giữ </option>
                  </select>
                </form>
                @endif
                @endforeach
